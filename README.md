...
In the phase 1 of our project, we have downloaded tweets from twitter and extracted urls and hashtags on the same. We have ran a python code to extracted the tweets from the out.json file and the extracted tweets and hashtags are stored in an file "input_data.txt" which is also uploaded in the repository. 

Hadoop:

We ran wordcount program on hadoop on the extracted file and ran the command " bin/hadoop jar share/hadoop/mapreduce/hadoop-mapreduce-examples-2.8.1.jar wordcount /Project_Phase_1/input_data.txt /Project_Phase_1/output_phase_1" and the ouput file for the same has been stored in the repository " part-r-00000 "
